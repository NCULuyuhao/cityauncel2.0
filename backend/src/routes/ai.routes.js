...
const { ensureStudentCoinBalance: ensureUserCoinBalance } = require("../services/users");
...
router.get("/api/ai-helper/status", authenticateToken, async (req, res) => {
  try {
    ...
    res.json({ unlocked: Boolean(unlockRow), unlockedAt: unlockRow?.unlockedAt || null, coins: Number(coinRow?.barrage_coins) || 0 });
  } catch (error) {
    console.error(error);
...
router.post("/api/ai-helper/unlock", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res.json({ message: isRenewal ? "AI 幫幫忙已續費" : "AI 幫幫忙已解鎖", unlocked: true, coins: coins - 1, credits: 2 });
  } catch (error) {
    await connection.rollback();
    console.error(error);
